...
namespace ViGo.Services
{
    public class TripMappingServices : BaseServices
    {
            ...
            ILogger logger)
        {
            ...
            Station startStation = await work.Stations.GetAsync(customerRoute.StartStationId.Value);
            Station endStation = await work.Stations.GetAsync(customerRoute.EndStationId.Value);
            ...
            foreach (Guid driverId in driverIds)
            {
                ...
                if (driverBookingDetails.Any())
                {
                    ...
                    IEnumerable<Guid> stationIds = customerRoutes.Select(r => r.StartStationId.Value)
                        .Concat(customerRoutes.Select(r => r.EndStationId.Value)).Distinct();
